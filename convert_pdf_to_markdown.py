...
class PDFToMarkdownConverter:
            ...
            for line in lines:
                ...
                if not line:
                    if current_line:
                        ...
                        current_line = ""
                    merged_lines.append("")
                elif len(line) < self.config['min_line_length'] and not self._is_header(line):
            ...
            with pdfplumber.open(pdf_path) as pdf:
                ...
                for page_num, page in enumerate(pdf.pages, 1):
                    ...
                    if text:
                        ...
                        if cleaned_text:
                            markdown_lines = self.process_text_block(cleaned_text, page_num)
    ...
    def batch_convert(self, input_dir: str, output_dir: Optional[str] = None) -> Dict[str, bool]:
...
Examples:
  ...
  %(prog)s -c config.json document.pdf   # Use custom config
    ...
    parser.add_argument('--no-page-numbers', action='store_true', help='Disable page number headers')
    parser.add_argument('--no-headers', action='store_true', help='Disable header detection')
    parser.add_argument('--no-lists', action='store_true', help='Disable list detection')
    parser.add_argument('--no-code', action='store_true', help='Disable code block detection')
    parser.add_argument('--min-line-length', type=int, default=40, help='Minimum line length for merging')
